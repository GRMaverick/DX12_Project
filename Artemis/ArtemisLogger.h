...
namespace ArtemisCore::Logging
{
	...
	public:
		static void SetSeverity(unsigned int _sev = SEVERITY_INFO);
		static void SetCategory(unsigned int _cat = CATEGORY_NONE);

		static void Log(unsigned int _sev, unsigned int _cat, const char* _pFormat, ...);
