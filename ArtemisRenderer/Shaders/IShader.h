...
namespace ArtemisRenderer::Shaders
{
	...
	public:
		...
		void SetConstantParameters(const Resources::ConstantBufferParameters& _params) 
		{ 
			m_ConstantParameters = _params; 
			//Resources::ConstantTable::Instance()->CreateConstantBuffersEntries(_params); 
		}

		const ShaderIOParameters GetShaderParameters(void) { return m_ShaderParameters; }
		const Resources::ConstantBufferParameters GetConstantParameters(void) { return m_ConstantParameters; }
	...
	protected:
		...
		Resources::ConstantBufferParameters m_ConstantParameters;
	};

	class Effect
	{
	public:
		void SetName(const char* _pName) { strncpy_s(m_pName, _countof(m_pName), _pName, _countof(m_pName)); }
