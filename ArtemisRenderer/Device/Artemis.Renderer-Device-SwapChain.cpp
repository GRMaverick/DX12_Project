...
namespace ArtemisRenderer::Device
{
	...
	SwapChain::~SwapChain(void)
	{
		if (m_pSwapChain)
		{
			//MemoryGlobalTracking::RecordExplicitDellocation(m_pSwapChain);
			m_pSwapChain->Release();
		}

		if (m_pDepthBuffer)
		{
			//MemoryGlobalTracking::RecordExplicitDellocation(m_pDepthBuffer);
	...
	{
		CD3DX12_RESOURCE_BARRIER tgtBarrier = CD3DX12_RESOURCE_BARRIER::Transition(m_pBackBuffers[m_CurrentBackBuffer], D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET, 0);
