...
namespace ArtemisRenderer::Device
{
	...
	{
		m_pAllocator->Reset();
		m_pList->Reset(m_pAllocator, nullptr);
	}
	...
	void CommandList::UpdateSubresource(ID3D12Resource* _pGPU, ID3D12Resource* _pCPU, UINT _intermediateOffset, UINT _firstSubresource, UINT _numSubresources, D3D12_SUBRESOURCE_DATA* _pSubresourceData)
	{
		...
		UpdateSubresources(m_pList, _pGPU, _pCPU, _intermediateOffset, _firstSubresource, _numSubresources, _pSubresourceData);
	}
	...
	{
		//ImGui::Render();
		//ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), m_pList);
	}
	...
	void CommandList::StartMarker(const char* _pFormatString)
	{
		// CPU
		//PIXBeginEvent(PIX_COLOR_DEFAULT, _pFormatString);

		// GPU
		//PIXBeginEvent(m_pList, PIX_COLOR_INDEX(0), _pFormatString);
	}

	void CommandList::EndMarker(void)
	{
		// CPU
		//PIXEndEvent();
