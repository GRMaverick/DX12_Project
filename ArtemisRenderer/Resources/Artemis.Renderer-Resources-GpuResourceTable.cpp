...
namespace ArtemisRenderer::Resources
{
	...
	void GpuResourceTable::Initialise(IShaderStage* _pVS, IShaderStage* _pPS)
	{
		...
		m_pSamplers = new SamplerState * [m_NumberSamplers];
	...
	bool GpuResourceTable::SetSamplerState(const char* _pName, SamplerState* _state)
	{
		ConstantBufferParameters cbParamsVS = m_pVertexShader->GetConstantParameters();
		for (unsigned int i = 0; i < cbParamsVS.NumberSamplers; ++i)
		{
	...
	unsigned long GpuResourceTable::GetSamplers(SamplerState*** _ppResources)
	{
